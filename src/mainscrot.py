...
class MainScrot:
    ...
    def initToolbar(self):
        ...
        self.actionShareButton = self.createActionButton("share", __("Tip share image"))
        self.actionShareButton.connect("button-press-event", lambda w, e: self.saveSnapshot(isShare=True))
        #self.actionShareButton.connect('toggled', lambda w: self.buttonToggled(w))
        self.actionShareButton.connect("button-release-event", lambda w, e: self.toggleReleaseEvent())
    ...
    def toggleReleaseEvent(self):
        buttonList = [self.actionRectangleButton, self.actionEllipseButton, self.actionArrowButton, self.actionLineButton,
                      self.actionTextButton, self.actionShareButton]
    ...
    def saveSnapshot(self, filename=None, filetype='png', isShare=False):
        '''Save snapshot.'''
        # Init cairo.
        cr = self.window.window.cairo_create()

        tipContent = "图片分享,需要一些时间"
        ...
        if filename == None:
            # Share snapshot
            if isShare:
                pixbuf.save('/tmp/tmp_eleveni386.png','png')
                cmd = 'python sharepic.py'
